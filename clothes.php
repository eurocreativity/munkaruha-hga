...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (validateCsrfToken($csrf)) {
        ...
        $wash_count = max(0, intval($_POST['wash_count'] ?? 0));
        $max_wash_count = max(1, intval($_POST['max_wash_count'] ?? 50));
        ...
        if ($action === 'create' && !empty($barcode) && !empty($name)) {
            ...
            $db->execute("
                INSERT INTO clothes (barcode, item_code, name, category, color, size, employee_id, location_id, status, notes, wash_count, max_wash_count)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ", [$barcode, $item_code, $name, $category, $color, $size, $emp_id, $location_id, $status, $notes, $wash_count, $max_wash_count]);
            ...
            if ($currentCloth['status'] === 'IN_LAUNDRY' || !empty($currentCloth['active_batch_number'])) {
                ...
                $db->execute("
                    UPDATE clothes SET barcode = ?, item_code = ?, name = ?, category = ?, color = ?, size = ?, employee_id = ?, location_id = ?, status = ?, notes = ?, wash_count = ?, max_wash_count = ?
                    WHERE id = ?
                ", [$barcode, $item_code, $name, $category, $color, $size, $emp_id, $location_id, $status, $notes, $wash_count, $max_wash_count, $clothId]);
    ...
    <form method="GET" action="clothes.php" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 pt-2">
      ...
      <div>
        <select name="category" onchange="this.form.submit()" class="w-full py-2 px-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500">
          ...
          <option value="Kabát" <?php echo $filterCat === 'Kabát' ? 'selected' : ''; ?>>Kabát</option>
          <option value="Mellény" <?php echo $filterCat === 'Mellény' ? 'selected' : ''; ?>>Mellény</option>
...
<!-- Munkaruha Létrehozás / Szerkesztés Modál -->
<div id="cloth-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs hidden p-4 overflow-y-auto">
  <div class="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full p-6 space-y-4 my-auto">
    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
      <h3 id="cloth-modal-title" class="text-lg font-bold text-slate-900">Munkaruha Szerkesztése</h3>
    ...
    <!-- Integritási figyelmeztető sáv (ha zárolt a tétel) -->
    <div id="laundry-lock-banner" class="p-3 bg-amber-50 border border-amber-200 rounded-xl hidden text-xs text-amber-900 flex items-start space-x-2">
      <i data-lucide="lock" class="w-4 h-4 text-amber-600 shrink-0 mt-0.5"></i>
      <div id="laundry-lock-text" class="space-y-1"></div>
    </div>

    <form method="POST" action="clothes.php" class="space-y-3 text-sm">
      <input type="hidden" name="csrf_token" value="<?php echo getCsrfToken(); ?>">
      <input type="hidden" name="form_action" id="cloth-form-action" value="update">
      <input type="hidden" name="cloth_id" id="cloth-form-id" value="">

      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="block text-xs font-bold text-slate-600 mb-1">Vonalkód (Azonosító) *</label>
          <input type="text" name="barcode" id="cloth-form-barcode" required class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 font-mono">
        </div>
        <div>
          <label class="block text-xs font-bold text-slate-600 mb-1">Cikkszám</label>
          <input type="text" name="item_code" id="cloth-form-item-code" placeholder="pl. 1010313" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-mono">
        </div>
      </div>

      <div>
        <label class="block text-xs font-bold text-slate-600 mb-1">Megnevezés *</label>
        <input type="text" name="name" id="cloth-form-name" required placeholder="pl. Férfi nadrág" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500">
      </div>
      ...
      <div class="grid grid-cols-3 gap-3">
        <div>
          ...
          <select name="category" id="cloth-form-category" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl">
            ...
            <option value="Mellény">Mellény</option>
        ...
        <div>
          ...
          <input type="text" name="size" id="cloth-form-size" placeholder="pl. 50, M, L" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-mono">
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <select name="location_id" id="cloth-form-location" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl">
        ...
        <div>
          ...
          <select name="status" id="cloth-form-status" class="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl">
            ...
            <option value="RESERVE">Tartalék (Raktárban)</option>
