...
if (!empty($rawToken)) {
    ...
    $user = $db->fetchOne("
        SELECT * FROM users 
        WHERE reset_token_hash = ? AND reset_token_expires_at > NOW() AND active = 1
        LIMIT 1
    ", [$tokenHash]);
    ...
    if ($user) {
        ...
        $message = "A megadott jelszó-visszaállító link érvénytelen vagy már lejárt (1 órán túl)! Kérjük igényeljen új linket.";
    }
} else {
    $message = "Hiányzó vagy érvénytelen biztonsági token!";
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $validToken) {
    ...
    if (validateCsrfToken($csrf)) {
        ...
        if (strlen($p1) < 6) {
            ...
            $db->execute("
                UPDATE users 
                SET password_hash = ?, reset_token_hash = NULL, reset_token_expires_at = NULL, email_verified = 1 
                WHERE id = ?
            ", [$hash, $user['id']]);

            $db->execute("
                INSERT INTO audit_logs (user_id, username, action, entity_type, entity_id, details)
                VALUES (?, ?, 'PASSWORD_RESET_SUCCESS', 'USER', ?, 'Jelszó sikeresen megváltoztatva email visszaállítással')
            ", [$user['id'], $user['username'], $user['id']]);

            setFlashMessage('success', 'A jelszava sikeresen megváltozott! Most már bejelentkezhet az új jelszavával.');
            redirect('login.php');
        }
    }
}
...
<head>
  ...
  <title>Új Jelszó Beállítása - <?php echo escape($companyName); ?></title>
...
<body class="h-full flex items-center justify-center p-4 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 font-sans text-slate-100">
  ...
  <div class="max-w-md w-full space-y-8 bg-white/5 backdrop-blur-xl p-8 rounded-3xl border border-white/10 shadow-2xl">
    ...
    <div class="text-center space-y-3">
      ...
      <h1 class="text-2xl font-black text-white tracking-tight">Új Jelszó Megadása</h1>
      <?php if ($user): ?>
        <p class="text-xs text-slate-400">Felhasználó: <strong class="text-white font-mono"><?php echo escape($user['username']); ?></strong> (<?php echo escape($user['full_name']); ?>)</p>
      <?php endif; ?>
    </div>
    ...
    <?php if ($validToken): ?>
      <!-- ŰRLAP -->
      <form method="POST" action="reset_password.php" class="space-y-4 text-sm">
        <input type="hidden" name="csrf_token" value="<?php echo getCsrfToken(); ?>">
        <input type="hidden" name="token" value="<?php echo escape($rawToken); ?>">

        <div>
          <label class="block text-xs font-semibold text-slate-300 mb-1.5">Új Jelszó (min. 6 karakter)</label>
          <div class="relative">
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-300 mb-1.5">Új Jelszó Újra</label>
          <div class="relative">
        ...
        <button type="submit" class="w-full py-3.5 px-4 bg-brand-600 hover:bg-brand-500 text-white font-bold rounded-xl shadow-lg shadow-brand-600/30 transition-all flex items-center justify-center space-x-2">
          ...
          <span>Jelszó Megváltoztatása & Belépés</span>
        </button>
      </form>
    <?php else: ?>
      <div class="text-center pt-2">
        <a href="forgot_password.php" class="px-5 py-2.5 bg-brand-600 hover:bg-brand-500 text-white text-xs font-bold rounded-xl shadow transition-all inline-flex items-center space-x-1.5">
          <i data-lucide="refresh-cw" class="w-4 h-4"></i>
          <span>Új visszaállító link kérése</span>
        </a>
      </div>
    <?php endif; ?>
